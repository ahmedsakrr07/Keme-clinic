...
userListStore.fetchUser(Number(userDatax.id)).then(response => {photo
...
const update = () => {
  axios.post('/team/edit/'+userDatax.id, {
    ...
    photo: photo.value,
    clinic_id: clinic_id.value,    email: email.value,
    phone: phone.value,
...
<template>
  ...
  >
   ...
   <VCol cols="12">
      <VBtn prepend-icon="tabler-plus" :to="{ name: 'doctor-add-appointments-id', params: { id: route.params.id },action: 'read',subject: 'home'}" v-if="userDatax.role =='Doctor'">
        {{ $t('Add Appointments') }}
      </VBtn><br><br>
